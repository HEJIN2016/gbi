...
var CONFIG = {
    URL: "/",
    NEWSURL: "/",
    //NEWSURL: "https://mtum.com/",
    USERURL: "/THBIT/",
    //USERURL: "https://mtum.com/THBIT/",
    webSocket: "wss://" + location.host + "/THBIT/portfolio",
    //webSocket: "wss://www.mtum.com/THBIT/portfolio",
    socketJS: "/THBIT/sockjs/portfolio"
};
...
var CookieUtil = {
    setCookie: function(name,value,expireDays){
        var date = Date.now() + expireDays*24*60*60*1000;//cookie过期时间
        date = new Date(date).toUTCString() ;
        document.cookie = name + "=" + encodeURIComponent(value) + ((!expireDays)?"":( "; expires=" + date));
    },//设置cookie
...
//分页函数
function renderPageList(page,pageNum,$page_list){
    page = parseInt(page);//current page num - 1
    pageNum = parseInt(pageNum);//total page num
    if (pageNum < 8) {
        for (var j = 0; j < pageNum; j++) {
            var str = '<li id=\"' + (j + 1) + '\"><a href="javascript:" data-page="'+ j + '\">' + (j + 1) + '</a></li>';
        ...
        if(page <= 4) {
            ...
            str += '<li id=\"' + (pageNum) + '\"><a href="javascript:" data-page="' + (pageNum - 1) + '\">' + (pageNum) + '</a></li>';
            $page_list.append(str);
        }else if(page > 4&&page < (pageNum - 5)){
            var str = '<li id="1"><a href="javascript:" data-page="0">1</a></li><li class="disabled" id="more_page"><a href="javascript:" class="more">...</a></li>';
            $page_list.append(str);
            for(var i = page - 3;i < page + 4;i++){
                var str = '<li id=\"' + (i + 1) + '\"><a href="javascript:" data-page="' + i + '\">' + (i + 1) + '</a></li>';
                $page_list.append(str);
            }
            str = '<li class="disabled" id="more_page"><a href="javascript:" class="more">...</a></li><li id=\"' + pageNum + '\"><a href="javascript:" data-page="' + (pageNum - 1) + '\">' + pageNum + '</a></li>';
            $page_list.append(str);
        }else {
            var str = '<li id="1"><a href="javascript:" data-page="">1</a></li><li class="disabled" id="more_page"><a href="javascript:" class="more">...</a></li>';
            $page_list.append(str);
    ...
    if (page != (pageNum - 1)) {
        var str = '<li><a href="javascript:" data-page="' + (page + 1) + '\" id="next_page">»</a></li>';
        $page_list.append(str);
    }
    if (page) {
        var str = '<li><a href="javascript:" data-page="' + (page - 1) + '\" id="prev_page">«</a></li>';
...
//获取随机数
function getRandomMultiple(state){
    if(state == 1){
        return Math.abs((Math.random()*2 - 1)/30);
    } else if(state == 0){
        return -Math.abs((Math.random()*2 - 1)/30);
    } else return (Math.random()*2 - 1)/30;
}
...
function judgeAmount(){
    //判断输入的金额是否有效
    var reg = /[^\d.]/g;
    if($amount.val().match(reg)){
        $("#errorAmount").show();
        $(".option-amount .btn").attr("disabled","disabled");
    } else if(parseFloat($amount.val()) > parseFloat($("#account").text())){
        $amount.val($("#account").text());
        $("#errorAmount").hide();
        $(".option-amount .btn").removeAttr("disabled");
        $("#winResult").text('+' + ($amount.val() * parseFloat($("#profit").text())/100).toFixed(2));
    } else {
        $("#errorAmount").hide();
        $(".option-amount .btn").removeAttr("disabled");
        $("#winResult").text('+' + ($amount.val() * parseFloat($("#profit").text())/100).toFixed(2));
...
//去除空格
function clearBalank(str){
    //str = str.replace(/\s+/g,"");
    str = str.replace(/&nbsp;/ig,'');
    str = str.replace(/&ensp;/ig,'');
    return str;
}
...
//与UTC时间差距
function gapTimeStamp(){
    return new Date().getTimezoneOffset()*60000;
}

//时间转换
function getDateDaytime(now){
    var time = '';
...
//秒时间
function getDateSecondTime(now){
    var date = '';
    var min = now.getMinutes();
    var hour = now.getHours();
...
function tableTd(num){
    return num?(parseFloat(num).toFixed(4)):sign;
}
function tableTd2(num){
    return num?(coinUnit + parseFloat(num).toFixed(4)):sign;
}
...
$(".mobile-btn").click(function(event){
    console.log('12');
    $(".mobile-list").css({width:'80%'});
    event.stopPropagation();
})
$("body,#close_mobile_list").click(function(){
    $(".mobile-list").css({width:'0'});
    $(".phone .sign-in-pad").hide();
})
...
function loginJudge(){
    if(CookieUtil.getCookie('loginName')&&CookieUtil.getCookie('token')){
        $("#user,.naver li.sign-in-list").show();
        //移动端
        $(".phoneSignUp,.phoneSinIn,#login-entry").hide();
        $(".phoneLogout").show();
        if(window.localStorage) {
            return {
                loginName: decryptByDES(CookieUtil.getCookie('loginName'), localStorage.getItem('random')),
                member_id: decryptByDES(CookieUtil.getCookie('member_id'), localStorage.getItem('random'))
            }
        } else {
            return {
                loginName: decryptByDES(CookieUtil.getCookie('loginName')),
                member_id: decryptByDES(CookieUtil.getCookie('member_id'))
            }
        }
    }else {
        //移动端
        $("#user,.phoneLogout,.naver li.sign-in-list").hide();
        $(".signup-btn,.login-btn,.phoneSignUp,.phoneSinIn").show();
        return false;
    }
}
...
window.alert = function(value) {
    $("#alert_modal .modal-body").html(value);
    $("#alert_modal").modal('show');
}

//User对象
function User(loginName,member_id){
    this.loginName = loginName;
    this.member_id = parseInt(member_id);
    var userBalance = 0;//余额 私有属性
    this.setBalance = function(balance){
        userBalance = balance;
        CookieUtil.setCookie('balance',balance,1);
    }
...
User.prototype.getToken = function(){
    if(window.localStorage){
        var key = localStorage.getItem('random');
        return decryptByDES(CookieUtil.getCookie('token_type'),key) + decryptByDES(CookieUtil.getCookie('token'),key);
    } else {
        return decryptByDES(CookieUtil.getCookie('token_type'),key) + decryptByDES(CookieUtil.getCookie('token'),key);
    }
}
...
//获取自选列表数据的函数
User.prototype.getOptionalData = function(){
    var data = CookieUtil.getCookie("optional");
...
//添加自选列表至服务器,
User.prototype.addOptionalList = function(coinArr){
    //var data = '{"member_id":' + this.member_id + ',"currency":' + coin + ',"exchange":' + exchange + ',"sortNo:"' + index + '}';
    //console.log(data);
    var data = {
        member_id: this.member_id,
        optionalList: coinArr
    };
    CookieUtil.setCookie("optional",JSON.stringify(coinArr));
    var params = JSON.stringify(data);
    var token = this.getToken();

    $.ajax({
        url: CONFIG.USERURL + 'userAction/addWebOptional.Action',
        //url: 'chart.html',
        type: 'POST',
        data: {
            "parms": params
    ...
    optionDataChange(coinArr);
}

//游戏货币单位
var gameCoinUnit = "Nano";

//获取并设置历史投注记录
User.prototype.getBettingHistory = function(page){
    if(!page) page = 0;
    var data = {
        member_id: this.member_id,
        pageNo: page
    ...
    $.ajax({
        url: CONFIG.USERURL + 'userAction/getTradeHistory.Action',
        type: 'GET',
        data: {
            "parms": params
        },
        ...
        success: function(data){
            ...
            if(total_num > 10){
                var pageNum = Math.ceil(total_num/10);
                $(".list-table .page").show();
                var $page_list = $(".list-table .page .pagination");
                $page_list.empty();
                renderPageList(page,pageNum,$page_list);
            } else {
                $(".list-table .page").hide();
            }
            var length = data.Data.length;
            if(length != 0){
                $("#historyTrade tbody").empty();
                for(var i = 0;i < length;i++){
                    var tradeData = data.Data[i];
                    var optionResult,symbol;

                    // 游戏结果
                    function gameResult(){
                        if (tradeData.winMoney == 0) {
                            optionResult = "Draw";
                            symbol = "";
                            return -1;
                        }//平局
                        else if(tradeData.winMoney > 0) {
                            optionResult = "Win";
                            symbol = "+";
                            return 1;
                        }//赢局
                    ...
                    gameResult();
                    var str = "<tr><td>" + tradeData.periodsno + "</td>";
                    str += "<td>" + getYearDateTime(new Date(tradeData.startdatetime * 1000)) + "</td>";
                    str += "<td>" + optionResult + "</td>";
                    str += "<td>" + gameCoinUnit + " " + tradeData.dealMoney + "</td>";
                    str += "<td>" + gameCoinUnit + " " + symbol + tradeData.winMoney + "</td></tr>";
                    $("#historyTrade tbody").append(str);
                }
            } else {
                $("#historyTrade tbody").empty().append('<tr><td>No transaction records.</td></tr>');
            }
        },
        error: function(){
            $("#loading_coin").hide();
            console.log('error');
            $(".list-area .error-info").show().html('Get history trade failed,please try again!');
        }
    })
}
...
User.prototype.signIn = function(){
    ...
    $.ajax({
        url: CONFIG.USERURL + 'userAction/memberSignIn.Action',
        type: 'GET',
        data: {
            "parms": params
        },
        ...
        success: function(data){
            if(data.message == "OK"||data.message == "ok") {
                data = JSON.parse(data.result);
                $(".coinAmount").html(data.Data.getMoney);
                if(document.documentElement.clientWidth > 756){
                    $(".pc .sign-in-pad").fadeIn();
                } else {
                    $(".phone .sign-in-pad").fadeIn();
                }
                console.log(data);
            } else if(data.messageContent == "00025"){
                alert('Sorry,You have signed in, please come back tomorrow.');
            } else {
                signErr();
            }
        },
...
//签到错误
function signErr(){
    alert('Sorry,sign in Error,please try again.');
}
$(".close-pad").click(function(){
    $(this).parent('.sign-in-pad').fadeOut();
})
...
function optionDataChange(coinArr) {
    if(coinArr.length != 0) {
        var marketArr = [];
        console.log(coinArr);
        //debugger;
        coinArr.forEach(function (item,index) {
            marketArr[index] = {};
            marketArr[index][item.currency] = item.exchange;
        })
       // console.log(marketArr);
        getOptionalData(marketArr);
    } else getOptionalData([]);
}

//获取自选列表所对应的数据
function getOptionalData(marketArr){
    if(marketArr.length != 0) {
        var data = JSON.stringify(marketArr);
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.length == 0){
                    ...
                    setOptionalTableArea(data);
                }
            }
        })
    } else {
        console.log('none market data');
        $("#loading_coin").hide();
        $("#optional_Table tbody").empty();
    }
}
...
if(!loginState) {
    ...
    loginUser = new User(loginState.loginName, loginState.member_id);
}
...
function indexOption() {
    var option = {
        ...
        tooltip: {
            ...
            borderColor: 'rgba(124,181,236,0.4)',
            borderWidth: 1,
            textStyle: {
                color: '#333'
            },
...
//二元期权option配置
function indexOption2() {
    var option = {
        //animation: true,
        //animationThreshold: 400000,
        ...
        yAxis: [{
            type: 'value',
            //min: function (value) {
            //    return parseInt(Math.floor(value.min));
            //},
            //max: function (value) {
            //    return parseInt(Math.floor(value.max));
            //},
            min: function (value) {
                return parseInt((Math.floor(value.min) - 2500)/1000) * 1000;
            },
            max: function (value) {
                return parseInt((Math.floor(value.max) + 2500)/1000) * 1000;
            },
            axisLine: {
                lineStyle: {
                    color: '#cacaca',//轴线的颜色
...
function GBIGrapth(GBIChart,color){
    ...
    $.ajax({
        ...
        success: function(data) {
            GBIChart.hideLoading();
            data = JSON.parse(data);
            var len = data.length;
            if(!data||!len){
                console.log('no gbi index data!');
            }else{
                var time = [];
                var value = [];
                    ...
                    series: [{
                        ...
                        areaStyle: {
                            normal: {
                                //color: '#D8E8F8'
                                //color: '#e7f2fe'
                                color: color?color:'rgba(231,242,254,0.8)'
                            }
                        },
                        lineStyle: {
                            normal: {
        ...
        error: function(){
            // $(".select-index").html('No Data');
            console.log('get gbi index data error!');
        }
    })
}
...
$('#login-entry,#phone-login-entry').on('click',function(e){
    loginEvent();
})
...
function loginEvent(){
    ...
    $('.login-sign-pad,.login-sign-mask').css('display','block');
...
var lang = (location.href.indexOf('cn.html')!==-1);
//登录
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.message == "OK"){
                    ...
                    if(!window.localStorage){
                        var obj = [
                            ["loginName",encryptByDES(result.Data.loginName),1],
                            ["nickName",encryptByDES(result.Data.nickname),1],
                            ["phoneNumber",encryptByDES(result.Data.phoneNumber),1],
                            ["token",encryptByDES(token),1],
                            ["token_type",encryptByDES(token_type),1],
                            ["member_id",encryptByDES(result.Data.member_id.toString()),1],
                            ["optional",JSON.stringify(result.Optional),1],
                            ["balance",result.Data.balance,1]
                        ];
                        CookieUtil.setCookies(obj);
                    } else{
                        localStorage.setItem('random',getRamNumber());
                        console.log(result.Data);
                        var key = localStorage.getItem('random');
                        var passObj = [
                            ["loginName",encryptByDES(result.Data.loginName,key),1],
                            ["token",encryptByDES(token,key),1],
                            ["token_type",encryptByDES(token_type,key),1],
                            ["member_id",encryptByDES(result.Data.member_id.toString(),key),1],
                            ["optional",JSON.stringify(result.Optional),1],
                            ["nickName",encryptByDES(result.Data.nickname,key),1],
                            ["phoneNumber",encryptByDES(result.Data.phoneNumber,key),1],
                            ["balance",result.Data.balance,1]
                        ];
                        CookieUtil.setCookies(passObj);
                    }
                    location.href = location.href;
                } else if(data.messageContent == "00004"||data.messageContent == "00012"){
                    $loginErr.html(lang?'手机号码未注册！':'The phone number is not registered!').show();
                    $("#login").html(translateW('登录','login')).removeAttr('disabled');
                    return false;
                } else if(data.messageContent == "000013"){
                    $loginErr.html(lang?'账户被锁定，请联系管理员！':'Your account has been locked, you can not log in, please contact your system administrator!').show();
                    $("#login").html(translateW('登录','login')).removeAttr('disabled');
                    return false;
                } else if(data.messageContent == "00014"){
                    $loginErr.html(lang?'密码输入次数超过五次，请找回密码！':'Enter incorrect password more than five times, please retrieve the password!').show();
                    $("#login").html(translateW('登录','login')).removeAttr('disabled');
                    return false;
                } else if(JSON.parse(data.messageContent).ERROR_00015=="00015"){
                    $loginErr.html((lang?'密码不正确，你还有':'The password is incorrect, you still have ') + JSON.parse(data.messageContent).hasErrorTimes + (lang?'次机会。如果你忘了密码，请重置它。':' chances. If you forgot your password, please retrieve it.')).show();
                    $("#login").html(translateW('登录','login')).removeAttr('disabled');
                    return false;
                } else {
            ...
            error: function(err){
                console.log(err);
                $("#login").html(translateW('登录','login')).removeAttr('disabled');
                $loginErr.html(lang?'登录失败，请重试！':'login failed,please try again!').show();
            }
        })
    }
})
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.message=='OK'){
                    ...
                    $phoneErr.html(lang?'一分钟内只能发送一次验证码！':'Only 1 Verification code can be sent in 1 minute!').show();
                    $getCodeBtn.removeAttr('disabled')
                } else if(data.messageContent == "00017"){
                    $phoneErr.html(lang?'此号码已被注册！':'The current phone number has been registered!').show();
            ...
            error: function(err){
                console.log(err);
                clearInterval(timer);
                $getCodeBtn.val(lang?'发送':'Send').removeAttr('disabled')
                $phoneErr.html(lang?'失败，请重试':'Failed, please try again later!').show();
            }
        })
    }else{
        return false;
                ...
                if(data.message == "OK"){
                    resetTimer = setInterval(function(){
                        if(time != 0) {
                            ...
                            $resetGetCodeBtn.val(lang?'发送':'Send').removeAttr('disabled').click(getResetCode);
                    ...
                    $("#resetPhoneErr").html(lang?'此号码已注册！':'This phone number is not registered yet!').show();
                }
                else if(data.messageContent == "00007"){
                    $("#resetCodeErr").html(lang?'一分钟只能发送一次验证码！':'Only 1 Verification code can be sent in 1 minute!').show();
                } else if(data.messageContent == "00017"){
                    $("#resetCodeErr").html(lang?'手机号码已被注册！':'The current phone number has been registered!').show();
                } else if(data.messageContent == "00020") {
                    $("#resetCodeErr").html(lang?'验证码已过期！':'The Verification code has expired!').show();
                } else{
                    $("#resetCodeErr").html(lang?'失败，请重试！':'Failed, please try again later!').show();
                }
                clearInterval(resetTimer);
                $resetGetCodeBtn.val(lang?'发送':'Send').removeAttr('disabled').click(getResetCode);
            },
                ...
                if(data.message == "OK"){
                    ...
                    if(!window.localStorage){
                        var obj = [
                            ...
                            ["token_type",encryptByDES(token_type),1],
                            ["member_id",encryptByDES(result.Data.member_id.toString()),1],
                            ["optional",JSON.stringify({optional: []}),1],
                            ["balance",result.Data.balance,1]
                        ];
                        CookieUtil.setCookies(obj);
                    } else{
                    ...
                    $emailErr.html(lang?'验证码不正确，请重试！':'Verification code is incorrect, please try again!').show();
                } else if(data.messageContent == "00020"){
                    $emailErr.html(lang?'验证码过期，请重试！':'The Verification code has expired, please try again!').show();
                } else{
                    $emailErr.html(lang?'错误，请重试！':'Error, please try again!').show();
                }
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.message == "OK"){
                    ...
                    $("#resetCodeErr").html(lang?'验证码不正确，请重试！':'Verification code is incorrect, please try again!').show();
                } else if(data.messageContent == "00008"){
                    $("#resetCodeErr").html(lang?'验证码无效，请重试！':'Verification code is invalid, please try again!').show();
                } else if(data.messageContent == "00012"){
                    $("#resetCodeErr").html(lang?'手机号码未注册！':'This phone number is not registered yet!').show();
                } else if(data.messageContent == "00019"){
                    $("#resetCodeErr").html(lang?'验证码不正确，请重试！':'Incorrect Verification code, please try again!').show();
                } else if(data.messageContent == "00020"){
                    $("#resetCodeErr").html(lang?'验证码已过期！':'The Verification code has expired, please try again!').show();
                } else{
                    $("#resetCodeErr").html(lang?'错误，请重试！':'Error, please try again!').show();
                }
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.message == "OK"){
                ...
                $codeErr.html(lang?'验证码不正确，请重试！':'Verification code is incorrect, please try again!').show();
            } else if(data.messageContent == "00008"){
                $codeErr.html(lang?'验证码无效，请重试！':'Verification code is invalid, please try again!').show();
            }  else if(data.messageContent == "00019"){
                $codeErr.html(lang?'验证码不正确，请重试！':'Incorrect Verification code, please try again!').show();
            } else if(data.messageContent == "00020"){
                $codeErr.html(lang?'验证码已过期！':'The Verification code has expired, please try again!').show();
            } else{
                $codeErr.html(lang?'错误，请重试！':'Error, please try again!').show();
            }
...
$(function(){
    // 取得整个验证码
    $('.inner-code').on('click',function(){
        $('.mask-input').focus();
        console.log(1);
    })
    var $mask_input = $('.mask-input');

    $mask_input.get(0).addEventListener('input', function(){
        console.log(1);
        if((!$(this).val().match(/^\d*$/))||$(this).val().length>4){
            $(this).val($(this).val().slice(0, -1))
        }
        if($(this).val().length==4){
            $('.btn-next').removeAttr('disabled').removeClass('disabled')
        }else {
            $('.btn-next').addClass('disabled').attr('disabled');
        }
        $('#code-1').html($(this).val().charAt(0));
        $('#code-2').html($(this).val().charAt(1));
        $('#code-3').html($(this).val().charAt(2));
        $('#code-4').html($(this).val().charAt(3));
    })
    function getVCode(){
        return $('#code-1').html()+$('#code-2').html()+$('#code-3').html()+$('#code-4').html();
        //return $("#mask-input").val();
    }
