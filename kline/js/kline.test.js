...
DarkTheme.prototype.__construct = function () {
    ...
    if (KlineIns.reverseColor) {
        this._colors[Theme.Color.Positive] = "#990e0e";//红色
        this._colors[Theme.Color.Negative] = "#19b34c";//绿色
...
$('body').on('click', '#sizeIcon', function () {
    ...
    if (KlineIns.isSized) {
        console.log($(KlineIns.element).parents('iframe'));
        ...
        //$(KlineIns.element).addClass('klineAllScreen');
        ...
        //$(KlineIns.element).removeClass('klineAllScreen');
...
function RequestData(showLoading) {
    ...
    if (KlineIns.type == "socket" && KlineIns.socket){
        requestOverSocket();
    } else {
        requestOverHttp();
    }
...
function gapTimeStamp2(){
    return new Date().getTimezoneOffset()*60000;
}

function dataChange2(data) {
    var gapTime = gapTimeStamp2();
    var len = data.length;
    var obj = {
        success: true,
        data: {
    ...
    for(var i = 0;i < len;i++){
        var currentyData = data[i];
        var arr = [];
        arr.push(currentyData.timestamp * 1000 - gapTime);
        arr.push(parseFloat(currentyData.popen));
        arr.push(parseFloat(currentyData.high));
        arr.push(parseFloat(currentyData.low));
        arr.push(parseFloat(currentyData.pclose));
        arr.push(parseFloat(currentyData.amount));
        obj.data.lines.push(arr);
    }
    console.log(obj);
    return (obj);
}
        ...
        KlineIns.G_HTTP_REQUEST = $.ajax({
            ...
            success: function (res) {
                if (KlineIns.G_HTTP_REQUEST) {
                    requestSuccessHandler(dataChange2(res));
                }
            },
